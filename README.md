...
- 当前状态：已落入最小 Worker daemon 骨架、平台 worker client 与 `worker-node run` CLI，并开始通过 `file:../themis-contracts` 依赖消费共享节点协议契约
...
- `src/cli/worker-node-main.ts` 已提供 `help` 与 `worker-node run --once` 最小 CLI
...
下一步应优先迁入 `doctor worker-node`，再把当前 echo executor 继续换成真实本机执行链。
